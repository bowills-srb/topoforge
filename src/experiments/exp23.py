"""Exp 23: Matched-budget confrontation dreams.
600 steps of pure old-at-zero (matching the construction budget).
Two arms: standard body vs gas body. 5 seeds.
Run: python src/exp23.py
"""
import numpy as np
import time
import sys
...
def run_arm(coords, cids, seed, arm_type):
    ...
    for t in range(TOTAL):
        p = (t // 20) % 3
        in_dream = (arm_type in ("confront", "confront_gas") and DREAM_START <= t < DREAM_END)
        if in_dream:
            inp = rng3.uniform(0, 0.02, N)
            pat = PATTERNS[p]
            if (t % 20) < 5:
                for c in pat: inp[cids == c] += 0.35
        elif t < REV:
            inp = rng3.uniform(0, 0.02, N)
            if (t % 20) < 5:
        ...
        if (t + 1) % 40 == 0:
            ...
            if crossover_step is None and new_val > old: crossover_step = t + 1
            snapshots.append((t + 1, old, new_val, float(Rhat[0])))
    M = np.zeros((NC, NC), dtype=int)
    np.add.at(M, (cids[src], cids[dst]), 1)
    old = M[0, 3] + M[3, 0]; new_val = M[1, 4] + M[4, 1]
    sup = M.sum() - np.trace(M) - old - new_val
    energy = D2[src, dst].sum()
    return old, new_val, sup, energy, crossover_step, snapshots

if __name__ == "__main__":
    print("=" * 70)
    print("EXP 23: MATCHED-BUDGET CONFRONTATION")
    print("600 steps old-at-zero dreams (= construction budget)")
    print("2 arms x 5 seeds. Acquire 0-600. Dream 600-1200. Probe 1200-1400.")
    print("=" * 70)
    SEEDS = [0, 1, 2, 3, 4]
    arms = [("confront", "salt_clust", "confront"), ("confront+gas", "salt_gas", "confront_gas")]
    all_results = {}
    for arm_name, body_type, arm_code in arms:
        ...
        for s in SEEDS:
            ...
            old, nv, sup, energy, cross, snaps = run_arm(coords, cids, s, arm_code)
            results.append((old, nv, sup, energy, cross, snaps))
            cx = str(cross) if cross else "never"
            elapsed = time.time() - t0
            traj = ""
            for step_check in [600, 880, 1200, 1400]:
                matches = [sn for sn in snaps if sn[0] == step_check]
                if matches:
                    traj += " @{}:old={},Rhat={:.2f}".format(step_check, matches[0][1], matches[0][3])
            print("  {} seed {}: old={} new={} sup={} cross={} ({:.0f}s){}".format(
                arm_name, s, old, nv, sup, cx, elapsed, traj))
        ...
        print("  >> crossover: {}/{} seeds, mean step: {}\n".format(len(valid), len(SEEDS), cx_str))
    print("=" * 70)
    print("GROK'S DECISION RULE:")
    R = all_results["confront"]
    A = np.array([(r[0], r[1], r[2], r[3]) for r in R], dtype=float)
    baseline = 4435.0
    reduction_pct = (baseline - A[:, 0].mean()) / baseline * 100
    print("  Baseline (Exp 22 control): {:.0f}".format(baseline))
    print("  Matched-budget old mass: {:.0f} +/- {:.0f}".format(A[:, 0].mean(), A[:, 0].std()))
    print("  Reduction: {:.0f}%".format(reduction_pct))
    if reduction_pct > 50:
        print("  VERDICT: >50% -> ECONOMICS ARE THE STORY")
    else:
        print("  VERDICT: <50% -> SOMETHING DEEPER PROTECTS THE EMPIRE")
    print("\nBring this back to Grok.")
