"""Exp 32: The TrueNorth Retrospective (v2 — clean math)
60 cores in a 6x10 grid, 15 neurons per core = 900 neurons.
900 / 5 identities = 180 each. All divisions clean.
Run: python src/exp32_truenorth.py
"""
import numpy as np
import time
import sys
...
if __name__ == "__main__":
    print("=" * 74)
    print("EXP 32: THE TRUENORTH RETROSPECTIVE")
    print("{} cores ({}x{}), {} neurons/core, {} total.".format(
        N_CORES, CORES_X, CORES_Y, NEURONS_PER_CORE, N))
    print("3 placements x 3 seeds. Frozen->Plastic->Reversal.")
    print("=" * 74)
    SEEDS = [0, 1, 2]
    strategies = [
        ("random", "Random (unoptimized baseline)"),
        ("vlsi", "VLSI-optimized (segregated — IBM approach)"),
        ("topoforge", "TopoForge (interleaved — our principle)"),
    ]
    all_results = {}
    for strat_name, description in strategies:
        print("\n  STRATEGY: {} ({})".format(strat_name, description))
        seed_results = []
        for s in SEEDS:
            t0 = time.time()
            ...
            print("    seed {}: frozen[e={:,.0f}] plastic[t={} e={:,.0f}] reversal[old={} new={} e={:,.0f}] ({:.0f}s)".format(
                s, r["frozen"]["energy"],
                r["plastic"]["taught"], r["plastic"]["energy"],
                r["reversal"]["old_03"], r["reversal"]["new_14"],
                r["reversal"]["energy"], time.time() - t0))
        all_results[strat_name] = seed_results
    print("\n" + "=" * 74)
    print("PHASE 1: FROZEN INFERENCE (IBM's question)")
    print("{:>12} {:>12}".format("strategy", "wire_energy"))
    print("-" * 26)
    for sn, _ in strategies:
        e = np.mean([r["frozen"]["energy"] for r in all_results[sn]])
        print("{:>12} {:>10,.0f}".format(sn, e))
    print("\nPHASE 2: PLASTIC LEARNING (the question IBM never asked)")
    print("{:>12} {:>10} {:>12}".format("strategy", "taught", "wire_energy"))
    print("-" * 36)
    for sn, _ in strategies:
        t = np.mean([r["plastic"]["taught"] for r in all_results[sn]])
        e = np.mean([r["plastic"]["energy"] for r in all_results[sn]])
        print("{:>12} {:>10.0f} {:>10,.0f}".format(sn, t, e))
    print("\nPHASE 3: WORKLOAD CHANGE (the question no one asks yet)")
    print("{:>12} {:>8} {:>8} {:>12}".format("strategy", "old_03", "new_14", "wire_energy"))
    print("-" * 42)
    for sn, _ in strategies:
        o = np.mean([r["reversal"]["old_03"] for r in all_results[sn]])
        n = np.mean([r["reversal"]["new_14"] for r in all_results[sn]])
        e = np.mean([r["reversal"]["energy"] for r in all_results[sn]])
        print("{:>12} {:>8.0f} {:>8.0f} {:>10,.0f}".format(sn, o, n, e))
    print("\n" + "=" * 74)
    print("THE CASE STUDY VERDICT:")
    print("  Phase 1: VLSI should win energy (IBM was right for frozen chips)")
    print("  Phase 2: TopoForge should win taught (our principle for plastic)")
    print("  Phase 3: TopoForge should adapt better after workload change")
