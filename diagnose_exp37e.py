...
print("Initial input<->output edges: {}".format(count_io_edges()))
print("Tracking bridge mass and Rhat across 15 full epochs:\n")
...
for epoch in range(15):
    ...
    for s_, d_ in zip(src, dst):
        s_, d_ = int(s_), int(d_)
        if s_ in in_set and (d_ in a_set or d_ in b_set):
            io_vals.append(V.get(s_, d_))
        elif d_ in in_set and (s_ in a_set or s_ in b_set):
            io_vals.append(V.get(d_, s_))
    io_vals = np.array(io_vals) if io_vals else np.array([0.0])
    print("  epoch {:>2}: bridge={:+10.2f}  Rhat={}  io_V min={:+.3f} max={:+.3f} "
          "n_positive={:>4}/{}".format(
              epoch, measure_bridge(), np.round(Rhat, 3),
              io_vals.min(), io_vals.max(), int((io_vals > 0).sum()), len(io_vals)))
